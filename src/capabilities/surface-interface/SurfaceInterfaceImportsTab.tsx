                  ...
                  <p class='text-xs text-neutral-400'>
                    ...
                    <code>
                      {entry.importKind === 'default-and-named'
                        ? `import ${entry.defaultAlias || 'DefaultExport'}, { ... } from '${entry.specifier || 'module'}';`
                        : `import ${entry.defaultAlias || 'DefaultExport'} from '${entry.specifier || 'module'}';`}
                    </code>
                  </p>
                </div>
...
function shouldAutoApplyImportKind(
  ...
  if (!summary.hasDefault && summary.named.length > 0 && (current === 'default' || current === 'default-and-named')) {
    return true;
  }
...
function describeModuleSummary(summary: ModuleExportSummary): string {
  ...
  if (summary.hasDefault && hasNamed) {
    return `Detected default export and ${summary.named.length} named export${summary.named.length === 1 ? '' : 's'}.`;
  }
...
function applyModuleAnalysis(
  ...
  if (shouldAutoApplyImportKind(entry.importKind, summary)) {
    ...
    if ((recommendation === 'default' || recommendation === 'default-and-named') && !next.defaultAlias.trim()) {
